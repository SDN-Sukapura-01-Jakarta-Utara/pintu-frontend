  ...
  <div>
    ...
    <form @submit.prevent="handleSubmit" class="w-full max-w-md rounded-2xl bg-white p-8 sm:p-10 md:p-12 shadow-2xl animate-slide-up mx-auto">
      ...
      <div class="mb-8 sm:mb-10 text-center">
        <h1 class="mb-1 sm:mb-2 text-2xl sm:text-3xl font-black text-red-600 title-bold">Portal Peserta Didik</h1>
        <p class="text-xs sm:text-sm font-medium text-pink-500">PINTU - Portal Informasi Terpadu SDN Sukapura 01</p>
      </div>
...
useHead({
  title: 'Login Peserta Didik | PINTU SDN Sukapura 01',
  link: [
    {
      rel: 'icon',
      type: 'image/jpeg',
...
const handleSubmit = async () => {
  ...
  try {
    const { studentAuthService } = await import('~/services/auth-student')
    ...
    if (response.status === 'success') {
      ...
      localStorage.setItem('student_permissions', JSON.stringify(response.data.permissions))
      localStorage.setItem('student_expires_at', response.data.expires_at)
      
      showAlert('success', 'Login Berhasil', `Selamat datang, ${response.data.student.nama}`)
      
      setTimeout(() => {
        router.replace('/peserta-didik')
      }, 1500)
    } else {
    ...
    showAlert('error', 'Login Gagal', error.message || 'Username atau password salah')
