<template>
  <div class="font-poppins bg-white overflow-x-hidden">
    ...
    <!-- ===== TENDIK CARDS ===== -->
    <section class="tendik-section">
      <div class="section-inner">
        <div class="section-header reveal" data-delay="0">
          <div class="section-eyebrow">Kepegawaian</div>
          <h2 class="section-title crimson-text">Daftar Tenaga Kependidikan</h2>
          <p class="section-desc">Tenaga kependidikan yang profesional dalam mendukung kegiatan sekolah</p>
        </div>

        <!-- Loading State -->
        <div v-if="loadingTendik" class="text-center py-12 text-gray-500">
          <i class="fas fa-spinner fa-spin text-3xl mb-3"></i>
          <p>Memuat data tenaga kependidikan...</p>
        </div>

        <!-- Error State -->
        <div v-else-if="errorTendik" class="text-center py-12 text-red-500">
          <i class="fas fa-exclamation-circle text-3xl mb-3"></i>
          <p>Gagal memuat data tenaga kependidikan</p>
          <p class="text-xs mt-2">{{ errorTendik.message }}</p>
        </div>

        <!-- Cards Grid -->
        <div v-else-if="tendikData && tendikData.length > 0" class="tendik-grid">
          <div 
            v-for="(tendik, index) in tendikData" 
            :key="index"
            class="tendik-card reveal"
            :data-delay="(index % 12) * 50"
          >
            <!-- Photo -->
            <div class="tendik-photo-wrapper">
              <img 
                v-if="tendik.foto" 
                :src="getPhotoUrl(tendik.foto)" 
                :alt="tendik.nama"
                class="tendik-photo"
                @error="handleImageError"
              />
              <div v-else class="tendik-photo-placeholder">
                <i class="fas fa-user"></i>
              </div>
            </div>

            <!-- Info -->
            <div class="tendik-info">
              <h3 class="tendik-name">{{ tendik.nama }}</h3>
              
              <div class="tendik-id">
                <i class="fas fa-id-card"></i>
                <span v-if="tendik.nip">NIP: {{ tendik.nip }}</span>
                <span v-else-if="tendik.nkki">NKKI: {{ tendik.nkki }}</span>
                <span v-else class="text-gray-400">-</span>
              </div>

              <div class="tendik-jabatan">
                <i class="fas fa-briefcase"></i>
                <span>{{ tendik.jabatan }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-else class="text-center py-12 text-gray-500">
          ...
          <p>Belum ada data tenaga kependidikan</p>
        </div>
      </div>
    </section>
...
import { getPublicDataTendik, getPublicDataKontak } from '~/services/public-home'

// Set page title
useHead({
  title: 'Tenaga Kependidikan - SDN Sukapura 01'
})
...
// Fetch tendik using useAsyncData
const { data: tendikData, pending: loadingTendik, error: errorTendik } = await useAsyncData(
  'tendik',
  async () => {
    try {
      const response = await getPublicDataTendik()
      console.log('Tendik response:', response)
...
const handleImageError = (event) => {
  ...
  event.target.parentElement.innerHTML = '<div class="tendik-photo-placeholder"><i class="fas fa-user"></i></div>'
}
...
.tendik-card:hover .tendik-photo {
