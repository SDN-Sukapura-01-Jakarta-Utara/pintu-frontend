  ...
  <div class="font-poppins bg-white overflow-x-hidden">
    ...
    <div v-if="loading" class="min-h-screen flex items-center justify-center bg-gray-50">
      <div class="text-center">
        ...
        <p class="text-gray-600 text-lg">Memuat pengumuman...</p>
      </div>
    </div>
    ...
    <div v-else-if="error" class="min-h-screen flex items-center justify-center bg-gray-50">
      <div class="text-center px-6">
        ...
        <h2 class="text-2xl font-bold text-gray-900 mb-2">Pengumuman Tidak Ditemukan</h2>
        <p class="text-gray-600 mb-6">Pengumuman yang Anda cari tidak tersedia.</p>
        <button 
          @click="$router.push('/media-publikasi/pengumuman')"
    ...
    <!-- Content -->
    <div v-else-if="pengumuman" class="bg-gray-50 py-8 sm:py-12">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center gap-2 text-gray-500 text-xs sm:text-sm mb-6">
          ...
          <a href="/media-publikasi/pengumuman" class="hover:text-red-600 transition-colors">
            Pengumuman
          </a>
          <i class="fas fa-chevron-right text-[10px]"></i>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <article class="bg-white rounded-lg shadow-sm overflow-hidden">
              ...
              <div class="px-6 sm:px-8 pt-6 sm:pt-8 pb-4 border-b border-gray-100">
                ...
                <div class="flex flex-wrap items-center gap-4 text-sm text-gray-600">
                  ...
                  <span v-if="pengumuman.penulis" class="flex items-center gap-2">
              ...
              <!-- Featured Image -->
              <div v-if="pengumuman.gambar" class="px-6 sm:px-8 pt-6">
                <img 
              ...
              <div class="px-6 sm:px-8 py-8">
                <div class="prose prose-lg max-w-none article-content" v-html="pengumuman.deskripsi"></div>
              </div>

              <!-- Files Section -->
              <div v-if="pengumuman.files && pengumuman.files.length > 0" class="px-6 sm:px-8 pb-8">
                <div class="pt-6 border-t border-gray-200">
                  ...
                  <div class="space-y-3">
                    <a 
                      v-for="file in pengumuman.files" 
                      :key="file.id"
                      :href="file.url"
                      target="_blank"
          ...
          <div class="lg:col-span-1">
            <!-- Pengumuman Lainnya -->
            <div v-if="pengumumanLainnya.length > 0" class="bg-white rounded-lg shadow-sm p-6 sticky top-8">
              ...
              <div class="space-y-4">
                <div 
                  v-for="item in pengumumanLainnya.slice(0, 5)" 
                  :key="item.id"
                  class="group cursor-pointer"
                  @click="goToPengumuman(item.id)"
                >
...
import { getPublicDetailPengumuman, getPublicPengumumanLainnya, getPublicDataKontak } from '~/services/public-home'

useHead({
  title: 'Detail Pengumuman - SDN Sukapura 01',
  link: [
    {
      rel: 'icon',
      type: 'image/jpeg',
...
const fetchPengumumanDetail = async () => {
  ...
  try {
    ...
    const response = await getPublicDetailPengumuman(id)
    pengumuman.value = response
    
    // Fetch related pengumuman after getting the main pengumuman
    await fetchPengumumanLainnya(id)
  } catch (err) {
...
// Fetch pengumuman lainnya
const fetchPengumumanLainnya = async (currentId) => {
  try {
    const response = await getPublicPengumumanLainnya(currentId)
    pengumumanLainnya.value = response.data || []
  } catch (err) {
    console.error('Error fetching pengumuman lainnya:', err)
    pengumumanLainnya.value = []
  }
}
...
// Go to pengumuman detail
const goToPengumuman = (id) => {
  navigateTo(`/media-publikasi/pengumuman/${id}`)
}
