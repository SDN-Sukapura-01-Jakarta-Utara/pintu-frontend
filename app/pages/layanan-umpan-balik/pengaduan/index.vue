      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div class="text-center">
          <div class="inline-flex items-center gap-2 px-4 py-2 bg-white/20 backdrop-blur-sm rounded-full mb-4">
            ...
            <span class="text-white text-sm font-semibold">Layanan Pengaduan</span>
          </div>
          <h1 class="text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-4">
            Pusat Layanan Pengaduan
          </h1>
          <p class="text-lg text-white/90 max-w-2xl mx-auto">
            Sampaikan pengaduan Anda atau lacak status pengaduan yang telah diajukan
          </p>
        </div>
      </div>
    </section>
        ...
        <div class="bg-white rounded-b-2xl shadow-sm border border-gray-200 p-6 sm:p-8">
            ...
            <form @submit.prevent="handleSubmit" class="space-y-6">
              ...
              <div>
                <label class="block text-sm font-semibold text-gray-900 mb-3">
                  ...
                  Tipe Pelapor <span class="text-red-600">*</span>
                </label>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <label class="flex items-start gap-3 p-4 border-2 rounded-xl cursor-pointer transition-all"
                    :class="form.tipe_pelapor === 'anonim' ? 'border-red-500 bg-red-50' : 'border-gray-300 hover:border-red-300'">
                    <input
                      type="radio"
                      v-model="form.tipe_pelapor"
                      value="anonim"
                      class="mt-1 w-4 h-4 text-red-600 cursor-pointer"
                    />
                    <div class="flex-1">
                      <p class="font-semibold text-gray-900">Anonim</p>
                      <p class="text-xs text-gray-600 mt-1">Identitas Anda akan dirahasiakan dan tidak akan disimpan dalam sistem</p>
                    </div>
                  </label>
                  <label class="flex items-start gap-3 p-4 border-2 rounded-xl cursor-pointer transition-all"
                    :class="form.tipe_pelapor === 'teridentifikasi' ? 'border-red-500 bg-red-50' : 'border-gray-300 hover:border-red-300'">
                    <input
                      type="radio"
                      v-model="form.tipe_pelapor"
                      value="teridentifikasi"
                      class="mt-1 w-4 h-4 text-red-600 cursor-pointer"
                    />
                    <div class="flex-1">
                      <p class="font-semibold text-gray-900">Teridentifikasi</p>
                      <p class="text-xs text-gray-600 mt-1">Anda akan mengisi identitas lengkap untuk memudahkan tindak lanjut</p>
                    </div>
                  </label>
                </div>
              </div>

              <!-- Identitas (Hanya muncul jika Teridentifikasi) -->
              <div v-if="form.tipe_pelapor === 'teridentifikasi'" class="space-y-6">
                <!-- Nama Lengkap -->
                <div>
                  ...
                  <input
                    v-model="form.nama"
                    type="text"
                    :required="form.tipe_pelapor === 'teridentifikasi'"
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  ...
                  >
                    ...
                    <option value="Pelayanan Tata Usaha">Pelayanan Tata Usaha</option>
                    <option value="Kedisiplinan">Kedisiplinan</option>
                    <option value="Keamanan">Keamanan</option>
                    <option value="Pungutan Liar">Pungutan Liar</option>
                    <option value="Gratifikasi">Gratifikasi</option>
                    <option value="KKN (Korupsi, Kolusi, Nepotisme)">KKN (Korupsi, Kolusi, Nepotisme)</option>
              ...
              <div>
                <label class="block text-sm font-semibold text-gray-900 mb-2">
                  ...
                  Judul Pengaduan <span class="text-red-600">*</span>
                </label>
                <input
                  v-model="form.judul"
                  type="text"
              ...
              <div>
                <label class="block text-sm font-semibold text-gray-900 mb-2">
                  ...
                  Deskripsi Pengaduan <span class="text-red-600">*</span>
                </label>
                <textarea
              ...
              <div>
                <label class="block text-sm font-semibold text-gray-900 mb-2">
                  ...
                  Upload Bukti Pengaduan <span class="text-red-600">*</span>
                </label>
                <p class="text-xs text-gray-600 mb-3">Minimal 1 file, maksimal 3 file, masing-masing maksimal 5 MB</p>
                <input
                  ref="fileInput"
                  type="file"
                  multiple
                ...
                >
                  ...
                  <span class="font-medium">Klik untuk upload file bukti</span>
                </button>
                ...
                <!-- Warning if no files -->
                <p v-if="selectedFiles.length === 0" class="text-xs text-red-600 mt-2">
                  <i class="fas fa-exclamation-triangle mr-1"></i>
                  Wajib upload minimal 1 file bukti
                </p>
              </div>
              ...
              <div class="pt-4">
                <button
                  type="submit"
                  :disabled="isSubmitting || !form.tipe_pelapor || selectedFiles.length === 0"
                ...
                >
                  ...
                  <span>{{ isSubmitting ? 'Mengirim...' : 'Kirim Pengaduan' }}</span>
                </button>
              </div>
            </form>
          </div>
          ...
          <div v-else-if="activeTab === 'lacak'">
            ...
            <form @submit.prevent="handleTrackSearch" class="mb-6">
              ...
              <div class="flex gap-3">
                <input
                  v-model="trackingId"
                  type="text"
                  placeholder="Masukkan ID Tiket (contoh: PGD-20260507-3JR7)"
            ...
            <div v-else-if="trackingData" class="space-y-6">
              ...
              <div class="bg-white border-2 border-gray-200 rounded-xl p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <p class="text-xs text-gray-600 mb-1">Tipe Pelapor</p>
                    <p class="text-sm font-semibold text-gray-900">
                      <span 
                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                        :class="{
                          'bg-purple-100 text-purple-800': trackingData.tipe_pelapor === 'anonim',
                          'bg-blue-100 text-blue-800': trackingData.tipe_pelapor === 'teridentifikasi'
                        }"
                      >
                        {{ trackingData.tipe_pelapor === 'anonim' ? 'Anonim' : 'Teridentifikasi' }}
                      </span>
                    </p>
                  </div>
                  <div v-if="trackingData.tipe_pelapor === 'teridentifikasi' && trackingData.nama">
                    <p class="text-xs text-gray-600 mb-1">Nama</p>
                    <p class="text-sm font-semibold text-gray-900">{{ trackingData.nama }}</p>
                  </div>
                  <div v-if="trackingData.tipe_pelapor === 'teridentifikasi' && trackingData.email">
                    <p class="text-xs text-gray-600 mb-1">Email</p>
                    <p class="text-sm font-semibold text-gray-900">{{ trackingData.email }}</p>
                  </div>
                  <div v-if="trackingData.tipe_pelapor === 'teridentifikasi' && trackingData.telepon">
            ...
            <div v-else-if="!trackingResult" class="text-center py-12">
              ...
              <h3 class="text-lg font-semibold text-gray-900 mb-2">Lacak Pengaduan Anda</h3>
              <p class="text-gray-600">Masukkan ID Tiket untuk melihat status pengaduan</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div @click.stop class="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden transform transition-all animate-bounce-in">
          ...
          <div class="relative bg-gradient-to-r from-green-600 via-green-500 to-green-600 p-8 text-center overflow-hidden">
            ...
            <div class="relative">
              ...
              <h3 class="text-2xl font-bold text-white mb-2">Pengaduan Terkirim!</h3>
              <p class="text-white/90 text-sm">Pengaduan Anda telah berhasil dikirim</p>
            </div>
          </div>
            ...
            <div class="space-y-3 mb-6">
              <div class="flex items-start gap-3">
                ...
                <p class="text-sm text-gray-700">
                  <strong>Simpan ID Tiket ini</strong> untuk melacak status pengaduan Anda
                </p>
              </div>
              <div v-if="form.tipe_pelapor === 'teridentifikasi'" class="flex items-start gap-3">
                <i class="fas fa-envelope text-green-500 mt-1"></i>
                <p class="text-sm text-gray-700">
                  Tindak lanjut akan dikirimkan ke email Anda
                </p>
              </div>
              <div class="flex items-start gap-3">
                <i class="fas fa-clock text-orange-500 mt-1"></i>
                <p class="text-sm text-gray-700">
                  Kami akan memproses pengaduan Anda dalam 1-3 hari kerja
                </p>
              </div>
            </div>
...
const handleSubmit = async () => {
  if (!form.value.tipe_pelapor) {
    showToast('warning', 'Pilih Tipe Pelapor', 'Silakan pilih tipe pelapor terlebih dahulu')
    return
  }

  if (selectedFiles.value.length === 0) {
    showToast('warning', 'Upload Bukti Diperlukan', 'Silakan upload minimal 1 file bukti pengaduan')
    return
  }
  ...
  try {
    ...
    formData.append('tipe_pelapor', form.value.tipe_pelapor)
    
    // Only append identity fields if teridentifikasi
    if (form.value.tipe_pelapor === 'teridentifikasi') {
    ...
    selectedFiles.value.forEach((file) => {
      formData.append('file_pengaduan', file)
    })
    
    const config = useRuntimeConfig()
    const response = await $fetch(`${config.public.apiBase}/api/v1/public/create-pengaduan`, {
      method: 'POST',
      body: formData
    })
    ...
    showToast('error', 'Gagal Mengirim', 'Gagal mengirim pengaduan. Silakan coba lagi.')
  } finally {
...
const handleTrackSearch = async () => {
  ...
  try {
    const config = useRuntimeConfig()
    const response = await $fetch(`${config.public.apiBase}/api/v1/public/track-pengaduan`, {
      method: 'POST',
      body: {
        id_tiket: trackingId.value.trim()
      }
    })
    ...
    console.error('Error tracking pengaduan:', error)
    trackingResult.value = 'not-found'
  } finally {
...
const getStatusTitle = (status: string) => {
  const titles: Record<string, string> = {
    pending: 'Pengaduan Dalam Antrian',
    processed: 'Pengaduan Sedang Diproses',
    closed: 'Pengaduan Telah Ditutup'
  }
...
const getStatusMessage = (status: string) => {
  const messages: Record<string, string> = {
    pending: 'Pengaduan Anda saat ini masih dalam antrian. Tim kami akan segera memprosesnya dalam 1-3 hari kerja.',
    processed: 'Pengaduan Anda sedang dalam proses penanganan oleh tim terkait. Kami akan segera menindaklanjuti.',
    closed: 'Pengaduan ini telah ditutup dan tidak dapat diproses lebih lanjut. Jika Anda memiliki pengaduan lain, silakan ajukan pengaduan baru.'
  }
