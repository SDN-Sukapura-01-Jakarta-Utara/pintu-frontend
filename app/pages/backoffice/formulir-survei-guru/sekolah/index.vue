      ...
      <div class="flex items-center justify-between gap-3 sm:gap-4 flex-wrap">
        <div>
          ...
          <p class="text-[13px] sm:text-sm md:text-[15px] text-gray-600 mt-1 sm:mt-2">
            Daftar formulir dan survei yang tersedia untuk guru
          </p>
        </div>
      </div>
    </div>
          ...
          <template #actions="{ item }">
            <div class="flex items-center justify-center">
              <button
                ...
                class="inline-flex items-center justify-center gap-1.5 px-3 sm:px-2.5 py-2 sm:pt-2.5 sm:pb-1.5 rounded-lg bg-gradient-to-br from-blue-50 to-blue-100 text-blue-700 font-semibold text-xs border border-blue-200 shadow-sm hover:shadow-md hover:from-blue-100 hover:to-blue-200 hover:-translate-y-0.5 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0 disabled:hover:shadow-sm cursor-pointer"
                :title="isFormOpen(item) ? 'Isi Formulir' : 'Formulir Ditutup'"
              >
                <i class="fa-solid fa-pen-to-square w-3.5 h-3.5 sm:w-5 sm:h-5"></i>
                <span v-if="isFormOpen(item)" class="hidden sm:inline">Isi Formulir</span>
              </button>
            </div>
          </template>
        </Table>
...
useHead({
  title: 'Form & Survei Sekolah | Backoffice',
})
