<template>
    <DashboardLayout>
        ...
        <ConfirmationDeleteModal 
            ...
            :message="`Apakah Anda yakin ingin menghapus pengaduan dari '${selectedPengaduan?.nama || 'Anonim'}' dengan ID Tiket '${selectedPengaduan?.id_tiket}'? Tindakan ini tidak dapat dibatalkan.`"
            ...
            <div class="flex items-center justify-between gap-3 sm:gap-4 flex-wrap">
                <div>
                    ...
                    <p class="text-[13px] sm:text-sm md:text-[15px] text-gray-600 mt-1 sm:mt-2">
                        Kelola pengaduan dari masyarakat
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            <div v-if="isLoading" class="flex items-center justify-center py-8 sm:py-12">
                <div class="flex flex-col items-center gap-3 sm:gap-4">
                    ...
                    <p class="text-sm sm:text-base text-gray-600 font-medium">Memuat data pengaduan...</p>
                </div>
            </div>
            ...
            <template v-else>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 mb-6">
                    <h3 class="text-base sm:text-lg font-semibold text-gray-900 mb-4">Filter Data Pengaduan</h3>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200">
                    <!-- Data Table -->
                    <div v-if="pengaduanList && pengaduanList.length > 0">
                        <Table 
                            :items="pengaduanList" 
                        ...
                        >
                            ...
                            <!-- Custom cell for tipe_pelapor -->
                            <template #cell-tipe_pelapor="{ item }">
                                <span 
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                                    :class="{
                                        'bg-purple-100 text-purple-800': item.tipe_pelapor === 'anonim',
                                        'bg-blue-100 text-blue-800': item.tipe_pelapor === 'teridentifikasi'
                                    }"
                                >
                                    {{ item.tipe_pelapor === 'anonim' ? 'Anonim' : 'Teridentifikasi' }}
                                </span>
                            </template>
                            ...
                            <!-- Custom cell for telepon -->
                            <template #cell-telepon="{ item }">
                                <span class="text-xs sm:text-sm md:text-[15px] text-gray-700">
                                    {{ item.telepon || '-' }}
                                </span>
                            </template>
                    ...
                    <div v-else class="flex flex-col items-center justify-center py-10 sm:py-16 px-4 sm:px-6">
                        <div class="h-16 w-16 sm:h-20 sm:w-20 rounded-full bg-gray-100 flex items-center justify-center mb-4 sm:mb-6">
                            <i :class="[
                                hasActiveFilters ? 'fa-solid fa-magnifying-glass' : 'fa-solid fa-bullhorn',
                                'text-2xl sm:text-4xl text-gray-400'
                            ]"></i>
                        </div>
                        <h3 class="text-base sm:text-lg font-semibold text-gray-900 mb-1">
                            {{ hasActiveFilters ? 'Data tidak ditemukan' : 'Belum ada pengaduan' }}
                        </h3>
                        <p class="text-sm sm:text-base text-gray-600 text-center mb-4 sm:mb-6 max-w-sm">
                            {{ hasActiveFilters ? 'Data tidak ditemukan dalam pencarian' : 'Belum ada pengaduan yang masuk dari masyarakat' }}
                        </p>
                    </div>
                </div>
            </template>
...
const pengaduanList = ref<any[]>([])
const totalData = ref(0)
const showDetailModal = ref(false)
const selectedPengaduan = ref<any>(null)
...
const tableColumns = [
    ...
    { key: 'tipe_pelapor', label: 'Tipe Pelapor', sortable: false },
...
const fetchData = async () => {
    ...
    try {
        ...
        const response = await fetch(`${config.public.apiBase}/api/v1/pengaduan/get-pengaduan`, {
            method: 'POST',
            credentials: 'include',
            headers: {
            ...
            body: JSON.stringify({
                search: {
                    ...
                    tipe_pelapor: filters.value.tipe_pelapor,
        ...
        pengaduanList.value = result.data || []
        totalData.value = result.pagination?.total || 0
    } catch (err: any) {
        error('Gagal Memuat Data', err.message || 'Terjadi kesalahan saat memuat data pengaduan')
        pengaduanList.value = []
        totalData.value = 0
    } finally {
...
const viewDetail = async (item: any) => {
    ...
    try {
        ...
        const response = await fetch(`${config.public.apiBase}/api/v1/pengaduan/get-pengaduan-by-id`, {
            method: 'POST',
            credentials: 'include',
            headers: {
                'Content-Type': 'application/json',
        ...
        if (!response.ok) {
            throw new Error('Failed to fetch pengaduan detail')
        }

        const result = await response.json()
        selectedPengaduan.value = result.data
    } catch (err: any) {
        error('Gagal Memuat Detail', 'Gagal memuat detail pengaduan')
...
const handleDelete = async () => {
    if (!selectedPengaduan.value) return
    ...
    try {
        ...
        const response = await fetch(`${config.public.apiBase}/api/v1/pengaduan/delete-pengaduan`, {
            method: 'POST',
            credentials: 'include',
            headers: {
                'Content-Type': 'application/json',
            ...
            body: JSON.stringify({ id: selectedPengaduan.value.id })
        })
        ...
        if (!response.ok) {
            throw new Error('Failed to delete pengaduan')
        }

        success('Pengaduan Berhasil Dihapus', 'Data pengaduan berhasil dihapus')
        ...
        const errorMessage = err.message || 'Gagal menghapus pengaduan'
