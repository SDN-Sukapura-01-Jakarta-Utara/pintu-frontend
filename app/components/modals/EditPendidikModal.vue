                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                            ...
                            <div>
                                <label
                                    ...
                                    NIP
                                    <span v-if="!form.isHonorer" class="text-red-600 ml-1">*</span>
                                </label>
                                <input v-model="form.nip" type="text" placeholder="Nomor Induk Pegawai"
                                    :disabled="isSubmitting || isNipNkkiDisabled || form.nkki.trim().length > 0"
                            ...
                            <!-- NKKI Input with Honorer Checkbox -->
                            <div>
                                <div class="flex items-center justify-between mb-2 sm:mb-3">
                                    ...
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input type="checkbox" v-model="form.isHonorer"
                                            :disabled="isSubmitting"
                                            class="w-4 h-4 text-red-600 rounded cursor-pointer" />
                                        <span class="text-xs sm:text-sm font-medium text-gray-700">Honorer</span>
                                    </label>
                                </div>
                                <input v-model="form.nkki" type="text" placeholder="Nomor KKI"
                                    :disabled="isSubmitting || isNipNkkiDisabled || form.nip.trim().length > 0"
                                    class="w-full rounded-lg border-2 border-gray-300 bg-white px-4 py-2 sm:py-3 text-xs sm:text-sm font-medium transition-all duration-200 placeholder-gray-400 focus:border-red-600 focus:bg-white focus:outline-none focus:ring-4 focus:ring-red-100 disabled:opacity-50 disabled:bg-gray-100 disabled:cursor-not-allowed" />
                            </div>
                        </div>
                            ...
                            <div>
                                ...
                                <div v-if="form.passwordType === 'auto'" class="space-y-3">
                                    ...
                                    <p v-if="!autoGeneratedPassword" class="text-xs text-amber-600">
                                        {{ form.isHonorer ? 'Masukkan username terlebih dahulu untuk generate password otomatis' : 'Masukkan NIP atau NKKI terlebih dahulu untuk generate password otomatis' }}
                                    </p>
                                </div>
...
const autoGeneratedPassword = computed(() => {
    if (form.value.passwordType === 'auto') {
        // If honorer, use username; otherwise use NIP or NKKI
        if (form.value.isHonorer) {
...
const isNipNkkiDisabled = computed(() => {
    return form.value.isHonorer
})
...
const handleSubmit = async () => {
    ...
    if (!form.value.isHonorer && !form.value.nip.trim() && !form.value.nkki.trim()) {
        toastStore.showToast('error', 'Gagal menyimpan', 'NIP atau NKKI harus diisi')
        return
    }
...
const initializeForm = async () => {
    if (props.pendidik) {
        ...
        // Check if NIP and NKKI are both empty - auto check honorer
        const nipEmpty = !props.pendidik.nip || props.pendidik.nip.trim() === ''
        ...
        form.value = {
            ...
            isHonorer: autoCheckHonorer ? true : (props.pendidik.is_honorer || false),
...
// Watch isHonorer checkbox - clear NIP and NKKI when checked
watch(
    () => form.value.isHonorer,
    (isHonorer) => {
        if (isHonorer) {
            form.value.nip = ''
            form.value.nkki = ''
