            ...
            <div
                ...
                <div
                    ...
                    <div class="relative z-10 flex-1">
                        <h2 class="text-lg sm:text-xl font-bold text-white">Tambah Kelas</h2>
                    </div>
...
const submitForm = async () => {
    ...
    try {
        const { createKelas } = await import('~/services/kelas')
        const requestBody = {
            name: form.value.name.trim(),
            status: form.value.status
        }

        console.log('Creating kelas with request body:', requestBody)
        await createKelas(requestBody)

        // Close modal
        emit('update:modelValue', false)
        emit('success', 'Kelas berhasil ditambahkan')

        resetForm()
    } catch (err: any) {
        const errorMessage = err.data?.message || err.data?.error || err.message || 'Gagal menambahkan kelas'
