            ...
            <div
                ...
                <div
                    ...
                    <div class="relative z-10 flex-1">
                        <h2 class="text-lg sm:text-xl font-bold text-white">Edit Kelas</h2>
                    </div>
                ...
                <div class="p-4 sm:p-8 relative z-10 overflow-y-auto flex-1">
                    ...
                    <form @submit.prevent="handleSubmit" class="space-y-4 sm:space-y-6">
                        ...
                        <div>
                            ...
                            <input v-model="form.name" type="text" placeholder="Contoh: 1" required
...
// Watch kelasData prop and populate form
watch(
    () => [props.kelasData, props.modelValue],
    ([newData, isOpen]) => {
        if (newData && isOpen) {
            form.value = {
                name: newData.name,
...
const handleSubmit = async () => {
    ...
    if (!form.value.name.trim()) {
        submitError.value = 'Nama kelas wajib diisi'
        return
    }

    if (!props.kelasData) {
        submitError.value = 'Data kelas tidak ditemukan'
        return
    }
...
const submitForm = async () => {
    if (!props.kelasData) return

    isSubmitting.value = true
    submitError.value = null

    try {
        const { updateKelas } = await import('~/services/kelas')
        const requestBody = {
            id: props.kelasData.id,
            name: form.value.name.trim(),
            status: form.value.status
        }

        console.log('Updating kelas with request body:', requestBody)
        await updateKelas(requestBody)

        // Close modal
        emit('update:modelValue', false)
        emit('success', 'Kelas berhasil diperbarui')

        resetForm()
    } catch (err: any) {
        const errorMessage = err.data?.message || err.data?.error || err.message || 'Gagal mengupdate kelas'
