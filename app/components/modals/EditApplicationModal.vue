            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm pointer-events-auto overflow-hidden">
                ...
                <div class="p-6">
                    <p class="text-gray-700 text-sm sm:text-base mb-4">
                        Anda akan mengupdate aplikasi <span class="font-semibold">{{ form.nama }}</span> sebagai aplikasi yang <span class="font-semibold text-green-600">ditampilkan di jumbotron</span>.
                    </p>
                    <p class="text-gray-600 text-sm sm:text-base leading-relaxed">
                        Dengan menyimpan data ini, aplikasi sekolah lainnya yang aktif sebagai jumbotron akan otomatis dinonaktifkan. Apakah Anda ingin melanjutkan?
                    </p>
                </div>
            ...
            <div
                ...
                <div
                    ...
                    <div class="relative z-10 flex-1">
                        <h2 class="text-lg sm:text-xl font-bold text-white">Edit Aplikasi Sekolah</h2>
                    </div>
                ...
                <div class="p-4 sm:p-8 relative z-10 overflow-y-auto flex-1">
                    ...
                    <form v-else @submit.prevent="handleSubmit" class="space-y-4 sm:space-y-6">
                        ...
                        <div>
                            ...
                            <input v-model="form.nama" type="text" placeholder="Contoh: PPDB Mutasi" required
                        ...
                        <div>
                            ...
                            <input v-model="form.link" type="url" placeholder="https://example.com" required
                                :disabled="isSubmitting"
                                class="w-full rounded-lg border-2 border-gray-300 bg-white px-4 py-2 sm:py-3 text-xs sm:text-sm font-medium transition-all duration-200 placeholder-gray-400 focus:border-red-600 focus:bg-white focus:outline-none focus:ring-4 focus:ring-red-100 disabled:opacity-50 disabled:cursor-not-allowed" />
                        </div>

                        <!-- Jadikan Jumbotron Radio -->
                        <div>
                            <label class="block text-xs sm:text-sm font-semibold text-gray-900 mb-2 sm:mb-3">
                                Jadikan Jumbotron
                                <span class="text-red-600 ml-1">*</span>
                            ...
                            <div class="flex items-center gap-6">
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" :value="true" v-model="form.show_in_jumbotron"
                                        :disabled="isSubmitting"
                                        class="w-4 h-4 text-red-600 border-gray-300 focus:ring-red-500 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed" />
                                    <span class="text-xs sm:text-sm font-medium text-gray-700">Ya</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" :value="false" v-model="form.show_in_jumbotron"
                                        :disabled="isSubmitting"
                                        class="w-4 h-4 text-red-600 border-gray-300 focus:ring-red-500 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed" />
                                    <span class="text-xs sm:text-sm font-medium text-gray-700">Tidak</span>
                                </label>
                            </div>
                        </div>
...
watch(() => props.modelValue, async (newValue) => {
    if (newValue && props.applicationData) {
        await loadData()
    }
})
...
const loadData = async () => {
    ...
    try {
        const { getApplicationById } = await import('~/services/application')
        const response = await getApplicationById(props.applicationData.id)
        
        form.value = {
            id: response.data.id,
            nama: response.data.nama,
            link: response.data.link,
            show_in_jumbotron: response.data.show_in_jumbotron,
            status: response.data.status
        }
    } catch (err: any) {
        const errorMessage = err.data?.message || err.data?.error || err.message || 'Gagal memuat data aplikasi'
...
const handleSubmit = async () => {
    ...
    if (!form.value.nama.trim()) {
        submitError.value = 'Nama aplikasi wajib diisi'
        return
    }

    if (!form.value.link.trim()) {
        submitError.value = 'Link aplikasi wajib diisi'
        return
    }

    // If show_in_jumbotron is true, show confirmation
    if (form.value.show_in_jumbotron) {
        showConfirmation.value = true
        return
    }

    // If show_in_jumbotron is false, submit directly
    await submitForm()
}
...
const submitForm = async () => {
    ...
    try {
        ...
        const requestBody = {
            ...
            show_in_jumbotron: form.value.show_in_jumbotron,
            status: form.value.status
        }

        console.log('Updating application with request body:', requestBody)
        await updateApplication(requestBody)

        // Close modal
        emit('update:modelValue', false)
        emit('success', 'Aplikasi sekolah berhasil diperbarui')

        resetForm()
    } catch (err: any) {
        const errorMessage = err.data?.message || err.data?.error || err.message || 'Gagal memperbarui aplikasi'
        submitError.value = errorMessage
        emit('error', errorMessage)
