            ...
            <div
                ...
                <div
                    ...
                    <div class="relative z-10 flex-1">
                        <h2 class="text-lg sm:text-xl font-bold text-white">Edit Bidang Studi</h2>
                    </div>
                ...
                <div class="p-4 sm:p-8 relative z-10 overflow-y-auto flex-1">
                    ...
                    <form @submit.prevent="handleSubmit" class="space-y-4 sm:space-y-6">
                        <!-- Nama Bidang Studi Input -->
                        <div>
                            <label class="block text-xs sm:text-sm font-semibold text-gray-900 mb-2 sm:mb-3">
                                Nama Bidang Studi
                                <span class="text-red-600 ml-1">*</span>
                            </label>
                            <input v-model="form.name" type="text" placeholder="Contoh: Matematika" required
...
const props = defineProps<{
    modelValue: boolean
    bidangStudiData: BidangStudiData | null
}>()
...
const handleSubmit = async () => {
    ...
    if (!form.value.name.trim()) {
        submitError.value = 'Nama bidang studi wajib diisi'
        return
    }

    if (!props.bidangStudiData) {
        submitError.value = 'Data bidang studi tidak ditemukan'
        return
    }
...
const submitForm = async () => {
    if (!props.bidangStudiData) return

    isSubmitting.value = true
    submitError.value = null

    try {
        const { updateBidangStudi } = await import('~/services/bidang-studi')
        const requestBody = {
            id: props.bidangStudiData.id,
            name: form.value.name.trim(),
            status: form.value.status
        }

        console.log('Updating bidang studi with request body:', requestBody)
        await updateBidangStudi(requestBody)

        // Close modal
        emit('update:modelValue', false)
        emit('success', 'Bidang studi berhasil diperbarui')

        resetForm()
    } catch (err: any) {
        const errorMessage = err.data?.message || err.data?.error || err.message || 'Gagal mengupdate bidang studi'
