            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm pointer-events-auto overflow-hidden">
                ...
                <div class="p-6">
                    <p class="text-gray-700 text-sm sm:text-base mb-4">
                        Anda akan menyimpan aplikasi <span class="font-semibold">{{ form.nama }}</span> sebagai aplikasi yang <span class="font-semibold text-green-600">ditampilkan di jumbotron</span>.
                    </p>
            ...
            <div
                ...
                <div
                    ...
                    <div class="relative z-10 flex-1">
                        <h2 class="text-lg sm:text-xl font-bold text-white">Tambah Aplikasi Sekolah</h2>
                    </div>
...
const submitForm = async () => {
    ...
    try {
        const { createApplication } = await import('~/services/application')
        const requestBody = {
        ...
        console.log('Creating application with request body:', requestBody)
        await createApplication(requestBody)

        // Close modal
        emit('update:modelValue', false)
        emit('success', 'Aplikasi sekolah berhasil ditambahkan')

        resetForm()
    } catch (err: any) {
        const errorMessage = err.data?.message || err.data?.error || err.message || 'Gagal menambahkan aplikasi'
