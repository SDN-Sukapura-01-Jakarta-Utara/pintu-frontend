            ...
            <div
                ...
                <div
                    ...
                    <div class="relative z-10 flex-1">
                        <h2 class="text-lg sm:text-xl font-bold text-white">Detail Pengumuman</h2>
                    </div>
                ...
                <div v-if="isLoading" class="p-4 sm:p-6 lg:p-8 flex items-center justify-center flex-1">
                    <div class="flex flex-col items-center gap-3 sm:gap-4">
                        ...
                        <p class="text-xs sm:text-sm md:text-base text-gray-600 font-medium">Memuat detail pengumuman...</p>
                    </div>
                </div>

                <div v-else-if="announcement" class="p-3 sm:p-5 md:p-8 relative z-10 overflow-y-auto flex-1 space-y-6">
                    ...
                    <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg sm:rounded-xl border-2 border-blue-200 p-3 sm:p-4 md:p-6">
                        ...
                        <div class="flex items-start justify-between gap-2 sm:gap-3 mb-2 sm:mb-3">
                            <div class="min-w-0">
                                <h3 class="text-sm sm:text-lg md:text-xl font-bold text-gray-900 truncate">{{ announcement.judul }}</h3>
                            </div>
                            <div :class="[
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 mt-3 sm:mt-4">
                            <div class="flex items-center gap-2 sm:gap-2.5">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <p class="text-[11px] sm:text-xs font-semibold text-gray-900 truncate">{{ formatDate(announcement.tanggal) }}</p>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-2 sm:gap-2.5">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <p class="text-[11px] sm:text-xs font-semibold text-gray-900 truncate">{{ announcement.penulis }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-blue-300">
                            <span :class="[
                                ...
                                announcement.status_publikasi === 'published'
                                    ? 'bg-green-100 text-green-800'
                                    : 'bg-yellow-100 text-yellow-800'
                            ]">
                                <i :class="[
                                    announcement.status_publikasi === 'published' ? 'fa-solid fa-check-circle' : 'fa-solid fa-hourglass-end',
                                    'w-3 h-3'
                                ]"></i>
                                {{ announcement.status_publikasi === 'published' ? 'Published' : 'Draft' }}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-lg sm:rounded-xl border-2 border-gray-200">
                        ...
                        <div class="p-3 sm:p-4 md:p-6">
                            <div class="max-w-none text-gray-700 text-[13px] sm:text-sm leading-relaxed deskripsi-content"
                                v-html="sanitizeHtml(announcement.deskripsi)"></div>
                        </div>
                    </div>

                    <!-- Files -->
                    <div v-if="announcement.files && announcement.files.length > 0" class="bg-white rounded-lg sm:rounded-xl border-2 border-gray-200">
                        <div class="border-b border-gray-200 px-3 sm:px-4 md:px-6 py-2 sm:py-3">
                            <h3 class="text-sm sm:text-base md:text-base font-bold text-gray-900 flex items-center gap-2">
                        ...
                        <div class="divide-y divide-gray-200">
                            <a v-for="file in announcement.files" :key="file.id" :href="file.url" target="_blank"
                    ...
                    <div class="bg-white rounded-lg sm:rounded-xl border-2 border-gray-200 p-3 sm:p-4 md:p-6">
                        ...
                        <div class="space-y-2 text-xs sm:text-sm">
                            <div class="flex flex-col sm:flex-row sm:justify-between gap-0.5 sm:gap-2 py-2 border-b border-gray-100">
                                ...
                                <span class="font-medium text-gray-900">{{ formatDateTime(announcement.created_at) }}</span>
                            </div>
                            <div v-if="announcement.updated_at" class="flex flex-col sm:flex-row sm:justify-between gap-0.5 sm:gap-2 py-2">
                                <span class="text-gray-600">Diperbarui pada:</span>
                                <span class="font-medium text-gray-900">{{ formatDateTime(announcement.updated_at) }}</span>
                            </div>
                        </div>
                    </div>
                </div>
...
watch(
    () => props.modelValue,
    async (newVal) => {
        if (newVal && props.announcementId > 0) {
            await announcementStore.fetchAnnouncementById(props.announcementId)
        }
    }
)
