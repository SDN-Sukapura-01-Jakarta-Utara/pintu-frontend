            ...
            <div
                ...
                <div
                    ...
                    <div class="relative z-10 flex-1">
                        <h2 class="text-lg sm:text-xl font-bold text-white">Tambah Ekstrakurikuler</h2>
                    </div>
                    ...
                    <form @submit.prevent="handleSubmit" class="space-y-4 sm:space-y-6">
                        ...
                        <div>
                            ...
                            <div v-else class="flex flex-wrap gap-3">
                                <label v-for="kelas in filteredKelases" :key="kelas.id" class="flex items-center gap-2 cursor-pointer">
                                    <input 
                                        type="checkbox" 
                                        :value="kelas.id" 
                                    ...
                                    />
                                    <span class="text-xs sm:text-sm text-gray-900">{{ kelas.name }}</span>
                                </label>
                            </div>
...
// Computed: Get filtered active kelas only
const filteredKelases = computed(() => {
    return kelases.value.filter(k => k.status === 'active')
})
...
const submitForm = async () => {
    ...
    try {
        const { createEkstrakurikuler } = await import('~/services/ekstrakurikuler')
        const requestBody = {
        ...
        console.log('Creating ekstrakurikuler with request body:', requestBody)
        await createEkstrakurikuler(requestBody)

        // Close modal
        emit('update:modelValue', false)
        emit('success', 'Ekstrakurikuler berhasil ditambahkan')

        resetForm()
    } catch (err: any) {
        const errorMessage = err.data?.message || err.data?.error || err.message || 'Gagal menambahkan ekstrakurikuler'
        submitError.value = errorMessage
        emit('error', errorMessage)
        console.error('Error creating ekstrakurikuler:', err)
    } finally {
