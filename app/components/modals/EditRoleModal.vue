      ...
      <div
        ...
        <div
          ...
          <div class="relative z-10 flex-1">
            <h2 class="text-lg sm:text-xl font-bold text-white">Edit Role</h2>
          </div>
        ...
        <div class="p-4 sm:p-8 relative z-10 overflow-y-auto flex-1">
          ...
          <div v-if="isLoading" class="flex items-center justify-center py-8">
            <div class="flex flex-col items-center gap-3">
              <div class="h-8 w-8 animate-spin rounded-full border-4 border-gray-200 border-t-red-600">
              </div>
              <p class="text-sm text-gray-600 font-medium">Memuat data role...</p>
            </div>
          </div>
          ...
          <form v-else @submit.prevent="handleSubmit" class="space-y-4 sm:space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
              ...
              <div>
                ...
                <select v-else v-model.number="form.system_id" @change="handleSystemChange" required
                  ...
                  <!-- Placeholder or selected inactive system info -->
                  <option v-if="!form.system_id" :value="0" disabled>Pilih Sistem</option>
                  <option v-else-if="systems.some(s => s.id === form.system_id && s.status !== 'active')"
                    :value="form.system_id" disabled class="text-gray-400 italic">
                    {{ systems.find(s => s.id === form.system_id)?.nama }} - Nonaktif
                  </option>
                  <!-- Active systems only -->
                  <option v-for="system in systems.filter(s => s.status === 'active')" :key="system.id"
                    :value="system.id">
                    {{ system.nama }}
                  </option>
                </select>
            ...
            <!-- Permissions Section -->
            <div v-if="form.system_id !== 0" class="bg-white rounded-lg sm:rounded-xl border-2 border-gray-200 p-4 sm:p-6">
              <div class="flex items-center gap-2 mb-4 sm:mb-6">
                <svg class="w-4 sm:w-5 h-4 sm:h-5 text-red-600 flex-shrink-0" fill="none" stroke="currentColor"
                  viewBox="0 0 24 24">
                  ...
                  <div v-for="(permissions, groupName) in groupedPermissions" :key="groupName"
                    ...
                    <div class="flex items-center gap-2 mb-3">
                      ...
                      <div class="min-w-0 flex-1">
                        ...
                        <p class="text-[10px] sm:text-xs text-gray-600">{{ permissions.length }} izin</p>
                      </div>
                    </div>
                    ...
                    <div class="space-y-2 max-h-96 overflow-y-auto">
                      <label v-for="permission in permissions" :key="permission.id"
                        ...
                        <input type="checkbox" :value="permission.id" v-model.number="form.permission_ids"
                        ...
                        <div class="flex-1 min-w-0">
                          ...
                          <p class="text-[10px] sm:text-[11px] line-clamp-2"
                            :class="permission.status === 'inactive' ? 'text-gray-500' : 'text-gray-600'">{{ permission.description
                          }}</p>
                        </div>
                      </label>
                    </div>
...
import type { SystemData, UpdateRoleRequest, PermissionData, RoleData } from '~/types/RoleType'
...
// Form state
const form = ref<UpdateRoleRequest & { permission_ids: number[] }>({
  id: 0,
  name: '',
  description: '',
  system_id: 0,
...
// Original role data to track changes
const originalRole = ref<RoleData | null>(null)
...
const handleSystemChange = async () => {
  ...
  try {
    // Fetch permissions for selected system
...
const fetchRoleData = async () => {
  ...
  try {
    const response = await getRoleById(props.roleData.id)
    const roleData = response.data

    // Store original data
    originalRole.value = roleData

    form.value = {
      id: roleData.id,
      name: roleData.name,
      description: roleData.description,
      system_id: roleData.system_id,
      status: roleData.status,
      permission_ids: roleData.permissions ? roleData.permissions.map(p => p.id) : []
    }

    // Fetch ALL permissions for the system
    permissionsLoading.value = true
    try {
      const permResponse = await getPermissionsBySystem(roleData.system_id)
    ...
    // If role's system is inactive, add it to the list
    if (roleData.system && roleData.system.status !== 'active') {
      const systemExists = systems.value.some(s => s.id === roleData.system.id)
      if (!systemExists) {
        systems.value.push(roleData.system)
      }
    }
  } catch (err: any) {
    console.error('Error fetching role data:', err)
    submitError.value = 'Gagal memuat data role'
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    const requestBody: UpdateRoleRequest & { permission_ids?: number[] } = {
      id: form.value.id,
    ...
    console.log('Updating role with request body:', requestBody)
    ...
    // Close modal
    emit('update:modelValue', false)
    emit('success', 'Role berhasil diperbarui')

    resetForm()
  } catch (err: any) {
    const errorMessage = err.data?.message || err.data?.error || err.message || 'Gagal mengupdate role'
    submitError.value = errorMessage
