            ...
            <div
                ...
                <div
                    ...
                    <div class="relative z-10 flex-1">
                        <h2 class="text-base sm:text-lg md:text-xl font-bold text-white">Detail Permission</h2>
                    </div>
                ...
                <div class="p-3 sm:p-5 md:p-8 relative z-10 overflow-y-auto flex-1">
                    ...
                    <div v-if="isLoading" class="flex items-center justify-center py-12">
                        <div class="flex flex-col items-center gap-4">
                            ...
                            <p class="text-sm text-gray-600 font-medium">Memuat detail permission...</p>
                        </div>
                    </div>
                    ...
                    <!-- Content -->
                    <div v-else-if="permissionDetail" class="space-y-6">
                        <!-- Permission Basic Info Card -->
                        <div
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 mt-3 sm:mt-4">
                                <div class="flex items-center gap-2 sm:gap-2.5">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-[11px] sm:text-xs font-semibold text-gray-900 truncate">{{
                                            formatDate(permissionDetail.created_at) }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="flex items-center gap-2 sm:gap-2.5">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-[11px] sm:text-xs font-semibold text-gray-900 truncate">{{
                                            formatDate(permissionDetail.updated_at) }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <div class="flex items-start justify-between gap-2">
                                            <div class="min-w-0 flex-1">
                                                <p class="text-xs sm:text-sm font-bold text-gray-900">{{
                                                    permissionDetail.system?.nama || '-' }}</p>
                                                <p class="text-[11px] sm:text-xs text-gray-600 mt-0.5 line-clamp-2">{{
                                                    permissionDetail.system?.description || 'Sistem Informasi' }}</p>
                                            </div>
                                            <div v-if="permissionDetail.system" :class="[
                                                'px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-[11px] font-semibold whitespace-nowrap flex-shrink-0',
                                                permissionDetail.system.status === 'active'
                                                    ? 'bg-green-100 text-green-800'
                                                    : 'bg-red-100 text-red-800'
                                            ]">
                                                {{ permissionDetail.system.status === 'active' ? 'Aktif' : 'Nonaktif' }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
import { getPermissionById } from '~/services/permission'
...
const fetchPermissionDetail = async () => {
    ...
    try {
        const response = await getPermissionById(props.permissionId)
        permissionDetail.value = response.data || null
    } catch (err: any) {
        const errorMessage = err.data?.message || err.data?.error || err.message || 'Gagal memuat detail permission'
...
// Watch modelValue to load permission data when modal opens
