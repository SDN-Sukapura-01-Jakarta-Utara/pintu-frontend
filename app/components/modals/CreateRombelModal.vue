            ...
            <div
                ...
                <div
                    ...
                    <div class="relative z-10 flex-1">
                        <h2 class="text-lg sm:text-xl font-bold text-white">Tambah Rombel</h2>
                    </div>
                    ...
                    <form @submit.prevent="handleSubmit" class="space-y-4 sm:space-y-6">
                        ...
                        <div>
                            ...
                            <select v-model.number="form.kelas_id" required :disabled="isSubmitting || kelasLoading"
                                ...
                                <option v-for="kelas in kelases.filter(k => k.status === 'active')" :key="kelas.id" :value="kelas.id">
                                    {{ kelas.name }}
                                </option>
                            </select>
                        </div>
...
const submitForm = async () => {
    ...
    try {
        const { createRombel } = await import('~/services/rombel')
        const requestBody = {
        ...
        console.log('Creating rombel with request body:', requestBody)
        await createRombel(requestBody)

        // Close modal
        emit('update:modelValue', false)
        emit('success', 'Rombel berhasil ditambahkan')

        resetForm()
    } catch (err: any) {
        const errorMessage = err.data?.message || err.data?.error || err.message || 'Gagal menambahkan rombel'
        submitError.value = errorMessage
        emit('error', errorMessage)
        console.error('Error creating rombel:', err)
    } finally {
