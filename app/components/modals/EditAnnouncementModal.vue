            ...
            <div
                ...
                <div
                    ...
                    <div class="relative z-10 flex-1">
                        <h2 class="text-lg sm:text-xl font-bold text-white">Edit Pengumuman</h2>
                    </div>
                    ...
                    <form @submit.prevent="handleSubmit" class="space-y-4 sm:space-y-6">
                        ...
                        <div>
                            ...
                            <div v-if="existingFiles.length > 0" class="mb-4">
                                <p class="text-xs text-gray-600 mb-2 font-medium">File yang Sudah Ada:</p>
                                <div class="space-y-2">
                            ...
                            <div v-if="selectedFiles.length > 0" class="mb-4">
                                <p class="text-xs text-gray-600 mb-2 font-medium">File Baru:</p>
                                <div class="space-y-2">
...
import type { AnnouncementData, FileItem } from '~/types/AnnouncementType'
...
const props = defineProps({
    modelValue: Boolean,
    announcementData: Object as () => AnnouncementData | null,
})
...
// Watch for announcement data changes
watch(() => props.announcementData, (newData) => {
    if (newData) {
        formData.value = {
            judul: newData.judul,
            tanggal: newData.tanggal.split('T')[0], // Extract date only
            deskripsi: newData.deskripsi,
            penulis: newData.penulis,
            status_publikasi: newData.status_publikasi,
            status: newData.status,
        }
        imagePreview.value = newData.gambar
        existingFiles.value = newData.files || []
...
const removeExistingFile = (fileId: string) => {
    ...
    if (!filesToDelete.value.includes(fileId)) {
...
const handleSubmit = async () => {
    ...
    if (!imagePreview.value) {
        submitError.value = 'Gambar pengumuman harus ada'
        return
    }
    ...
    try {
        ...
        if (filesToDelete.value.length > 0) {
            data.append('files_to_delete', JSON.stringify(filesToDelete.value))
        }

        // Submit
        const result = await announcementStore.editAnnouncement(props.announcementData.id, data)
        ...
        const message = error.message || 'Terjadi kesalahan saat memperbarui pengumuman'
