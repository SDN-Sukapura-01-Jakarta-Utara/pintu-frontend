      ...
      >
        ...
        >
          ...
          <div class="relative z-10 flex-1">
            <h2 class="text-lg sm:text-xl font-bold text-white">Edit Struktur Organisasi</h2>
          </div>
...
const allPegawais = computed(() => {
  return kepegawaianStore.kepegawaians
})
...
const loadFormData = () => {
  ...
  form.value = {
    id: props.data.id,
    tipeInput: props.data.pegawai_id ? 'pegawai' : 'non-pegawai',
    pegawai_id: props.data.pegawai_id,
    nama_non_pegawai: props.data.nama_non_pegawai,
    jabatan_non_pegawai: props.data.jabatan_non_pegawai,
    urutan: props.data.urutan,
    relasi: props.data.relasi,
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await strukturOrganisasiStore.updateItem(submitData)
    toastStore.success('Berhasil', 'Struktur organisasi berhasil diperbarui')
    emit('success')
    closeModal()
  } catch (error: any) {
    toastStore.error('Gagal', error.message || 'Gagal memperbarui struktur organisasi')
    emit('error')
  } finally {
...
watch(
  ...
  async (newVal) => {
    if (newVal) {
      loadFormData()
      await kepegawaianStore.fetchKepegawaian(1, 1000, {})
    }
  }
)
</script>
