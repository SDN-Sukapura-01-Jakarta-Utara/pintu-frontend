            ...
            <div
                ...
                <div v-if="isLoading" class="p-4 sm:p-6 lg:p-8 flex items-center justify-center flex-1">
                    <div class="flex flex-col items-center gap-3 sm:gap-4">
                        ...
                        <p class="text-xs sm:text-sm md:text-base text-gray-600 font-medium">Memuat detail siswa...</p>
                    </div>
                </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 mt-3 sm:mt-4">
                            ...
                            <div class="flex items-center gap-2 sm:gap-2.5">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <p class="text-[11px] sm:text-xs font-semibold text-gray-900 truncate">{{
                                        siswa.tahun_pelajaran?.tahun_pelajaran || '-' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-lg sm:rounded-xl border-2 border-gray-200">
                        <div class="border-b border-gray-200 px-3 sm:px-4 md:px-6 py-2 sm:py-3">
                            <h3 class="text-sm sm:text-base font-bold text-gray-900 flex items-center gap-2">
                                <i class="fa-solid fa-id-card w-3.5 sm:w-4 h-3.5 sm:h-4 text-purple-600 flex-shrink-0"></i>
                        ...
                        <div class="p-3 sm:p-4 md:p-6">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                                ...
                                <div class="flex flex-col items-center">
                                    <p class="text-[11px] sm:text-xs text-gray-600 font-medium mb-2">Foto Siswa</p>
                                    <div v-if="siswa.peserta_didik?.photo" class="w-32 h-40 rounded-lg overflow-hidden border-2 border-gray-300 shadow-md">
                                        <img :src="siswa.peserta_didik.photo" alt="Foto Siswa" class="w-full h-full object-cover" />
                                    </div>
                                    <p v-else class="text-xs text-gray-500 italic">Belum ada foto</p>
                                </div>

                                <!-- Barcode -->
                                <div class="flex flex-col items-center">
                                    <p class="text-[11px] sm:text-xs text-gray-600 font-medium mb-2">Barcode</p>
                                    <BarcodeDisplay v-if="siswa.peserta_didik?.barcode" :value="siswa.peserta_didik.barcode" :size="120" />
                                    <p v-else class="text-xs text-gray-500 italic">Belum ada barcode</p>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                                <div>
                                    ...
                                    <p class="text-xs sm:text-sm font-semibold text-gray-900">{{
                                        siswa.peserta_didik?.jenis_kelamin === 'L' ? 'Laki-laki' : 'Perempuan' }}</p>
                                </div>
                                <div>
                                    <p class="text-[11px] sm:text-xs text-gray-600 font-medium">Tempat Lahir</p>
                                    <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ siswa.peserta_didik?.tempat_lahir || '-' }}</p>
                                </div>
                                <div>
                                    <p class="text-[11px] sm:text-xs text-gray-600 font-medium">Tanggal Lahir</p>
                                    <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ formatDate(siswa.peserta_didik?.tanggal_lahir) }}</p>
                                </div>
                                <div>
                                    <p class="text-[11px] sm:text-xs text-gray-600 font-medium">Agama</p>
                                    <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ siswa.peserta_didik?.agama || '-' }}</p>
                                </div>
                                <div>
                                    <p class="text-[11px] sm:text-xs text-gray-600 font-medium">NIK</p>
                                    <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ siswa.peserta_didik?.nik || '-' }}</p>
                                </div>
                                <div>
                                    <p class="text-[11px] sm:text-xs text-gray-600 font-medium">Username</p>
                                    <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ siswa.peserta_didik?.username || '-' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="p-3 sm:p-4 md:p-6">
                            <div class="grid grid-cols-1 gap-3 sm:gap-4">
                                <div>
                                    ...
                                    <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ siswa.peserta_didik?.alamat || '-' }}</p>
                                </div>
                                <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                                    <div>
                                        <p class="text-[11px] sm:text-xs text-gray-600 font-medium">RT</p>
                                        <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ siswa.peserta_didik?.rt || '-' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-[11px] sm:text-xs text-gray-600 font-medium">RW</p>
                                        <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ siswa.peserta_didik?.rw || '-' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-[11px] sm:text-xs text-gray-600 font-medium">Kelurahan</p>
                                        <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ siswa.peserta_didik?.kelurahan || '-' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-[11px] sm:text-xs text-gray-600 font-medium">Kecamatan</p>
                                        <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ siswa.peserta_didik?.kecamatan || '-' }}</p>
                                    </div>
                                </div>
                                <div>
                                    <p class="text-[11px] sm:text-xs text-gray-600 font-medium">Kode Pos</p>
                                    <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ siswa.peserta_didik?.kode_pos || '-' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="p-3 sm:p-4 md:p-6">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                                <div>
                                    ...
                                    <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ siswa.peserta_didik?.nama_ayah || '-' }}</p>
                                </div>
                                <div>
                                    <p class="text-[11px] sm:text-xs text-gray-600 font-medium">Nama Ibu</p>
                                    <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ siswa.peserta_didik?.nama_ibu || '-' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
