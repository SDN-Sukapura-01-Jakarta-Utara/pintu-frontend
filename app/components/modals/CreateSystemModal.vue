            ...
            <div
                ...
                <div
                    ...
                    <div class="relative z-10 flex-1">
                        <h2 class="text-lg sm:text-xl font-bold text-white">Tambah Sistem</h2>
                    </div>
...
const handleSubmit = async () => {
    ...
    try {
        const { createSystem } = await import('~/services/user')
        const requestBody = {
        ...
        console.log('Creating system with request body:', requestBody)
        await createSystem(requestBody)

        // Close modal
        emit('update:modelValue', false)
        emit('success', 'Sistem berhasil ditambahkan')

        resetForm()
    } catch (err: any) {
        const errorMessage = err.data?.message || err.data?.error || err.message || 'Gagal menambahkan sistem'
