            ...
            <div
                ...
                <div
                    ...
                    <div class="relative z-10 flex-1">
                        <h2 class="text-lg sm:text-xl font-bold text-white">Edit Ekstrakurikuler</h2>
                    </div>
                ...
                <div class="p-4 sm:p-8 relative z-10 overflow-y-auto flex-1">
                    ...
                    <form @submit.prevent="handleSubmit" class="space-y-4 sm:space-y-6">
                        <!-- Nama Ekstrakurikuler Input -->
                        <div>
                            <label class="block text-xs sm:text-sm font-semibold text-gray-900 mb-2 sm:mb-3">
                                Nama Ekstrakurikuler
                                <span class="text-red-600 ml-1">*</span>
                            </label>
                            <input v-model="form.name" type="text" placeholder="Contoh: Marawis" required
                        ...
                        <div>
                            ...
                            <div v-if="kelasLoading" class="flex items-center justify-center py-4">
                                <div class="h-5 w-5 animate-spin rounded-full border-2 border-gray-300 border-t-red-600"></div>
                            </div>
                            <div v-else class="flex flex-wrap gap-3">
                                <label v-for="kelas in allKelases" :key="kelas.id" class="flex items-center gap-2 cursor-pointer">
                                    <input 
                                        type="checkbox" 
                                        :value="kelas.id" 
                                        v-model.number="form.kelas_ids"
                                        :disabled="isSubmitting || isKelasDisabled(kelas.id)"
                                        class="w-4 h-4 rounded border-gray-300 text-red-600 focus:ring-2 focus:ring-red-100 disabled:opacity-50 disabled:cursor-not-allowed"
                                    />
                                    <span :class="[
                                        'text-xs sm:text-sm',
                                        isKelasDisabled(kelas.id) ? 'text-gray-400 line-through' : 'text-gray-900'
                                    ]">
                                        {{ kelas.name }}{{ isKelasDisabled(kelas.id) ? ' (Nonaktif)' : '' }}
                                    </span>
                                </label>
                            </div>
                            <p v-if="form.kelas_ids.length === 0" class="text-xs text-red-600 mt-1">
                        ...
                        <div>
                            ...
                            <select v-model="form.kategori" required :disabled="isSubmitting"
...
const props = defineProps<{
    modelValue: boolean
    ekstrakurikulerData: EkstrakurikulerData | null
}>()
...
const handleSubmit = async () => {
    ...
    if (!form.value.name.trim()) {
        submitError.value = 'Nama ekstrakurikuler wajib diisi'
        return
    }

    if (form.value.kelas_ids.length === 0) {
        submitError.value = 'Pilih minimal satu kelas'
        return
    }

    if (!form.value.kategori) {
        submitError.value = 'Kategori wajib dipilih'
        return
    }

    if (!props.ekstrakurikulerData) {
        submitError.value = 'Data ekstrakurikuler tidak ditemukan'
        return
    }
...
const submitForm = async () => {
    if (!props.ekstrakurikulerData) return

    isSubmitting.value = true
    submitError.value = null

    try {
        const { updateEkstrakurikuler } = await import('~/services/ekstrakurikuler')
        const requestBody = {
            id: props.ekstrakurikulerData.id,
            name: form.value.name.trim(),
            kelas_ids: form.value.kelas_ids,
            kategori: form.value.kategori as 'wajib' | 'tidak wajib',
            status: form.value.status
        }

        console.log('Updating ekstrakurikuler with request body:', requestBody)
        await updateEkstrakurikuler(requestBody)

        // Close modal
        emit('update:modelValue', false)
        emit('success', 'Ekstrakurikuler berhasil diperbarui')

        resetForm()
    } catch (err: any) {
        const errorMessage = err.data?.message || err.data?.error || err.message || 'Gagal mengupdate ekstrakurikuler'
