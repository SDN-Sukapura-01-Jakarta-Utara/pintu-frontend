            ...
            <div
                ...
                <div class="p-4 sm:p-6">
                    <div class="flex flex-col items-center text-center">
                        <!-- Info Icon -->
                        <div
                            class="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-blue-100 flex items-center justify-center mb-3 sm:mb-4">
                        ...
                        <!-- Title -->
                        <h3 class="text-base sm:text-lg font-bold text-gray-900 mb-2">Generate Barcode Siswa</h3>

                        <!-- Message -->
                        <p class="text-xs sm:text-sm text-gray-600 mb-4 sm:mb-6">
                            Apakah Anda yakin ingin generate barcode untuk semua siswa?
                            <br><br>
                            <span class="font-medium text-gray-900">
                                Barcode siswa yang sudah tergenerate tidak akan digenerate ulang. Hanya siswa yang belum memiliki barcode yang akan diproses.
                            </span>
                        </p>
                    </div>
                    ...
                    <div class="flex gap-3">
                        ...
                        <button type="button" @click="handleConfirm" :disabled="isLoading"
                            class="flex-1 flex items-center justify-center gap-2 px-4 py-2 sm:py-2.5 rounded-lg bg-blue-600 text-white font-semibold text-xs sm:text-sm hover:bg-blue-700 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer">
                            <span v-if="isLoading"
...
const handleConfirm = async () => {
    ...
    try {
        const response = await $fetch(`${config.public.apiBase}/api/v1/peserta-didik/generate-barcode-all-peserta-didik`, {
            method: 'POST',
            headers: {
        ...
        toastStore.success('Sukses', 'Barcode siswa berhasil digenerate')
        ...
        const errorMessage = error?.data?.message || error?.message || 'Gagal generate barcode siswa'
