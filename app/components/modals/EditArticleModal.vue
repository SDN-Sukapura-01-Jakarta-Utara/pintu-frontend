            ...
            <div
                ...
                <div
                    ...
                    <div class="relative z-10 flex-1">
                        <h2 class="text-lg sm:text-xl font-bold text-white">Edit Artikel</h2>
                    </div>
                    ...
                    <form @submit.prevent="handleSubmit" class="space-y-4 sm:space-y-6">
                        ...
                        <div>
                            ...
                            <div v-if="imagePreview" class="mb-3 sm:mb-4 relative group">
                                ...
                                <div v-if="!isNewImage" class="absolute top-2 left-2 px-2 py-1 bg-blue-600 text-white rounded text-xs font-semibold">
                                    Gambar Saat Ini
                                </div>
                            ...
                            <div v-if="!imagePreview" @click="$refs.imageInput.click()"
                                @dragover.prevent="isDraggingImage = true" @dragleave="isDraggingImage = false"
                                @drop.prevent="handleImageDrop" :class="[
                        ...
                        <div>
                            ...
                            <div v-if="existingFiles.length > 0" class="mb-4 space-y-2">
                                <p class="text-xs font-semibold text-gray-600 mb-2">File yang Ada:</p>
                                <div v-for="file in existingFiles" :key="file.id"
                                    class="flex items-center justify-between gap-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                            ...
                            <div v-if="selectedFiles.length > 0" class="mb-4 space-y-2">
                                <p class="text-xs font-semibold text-gray-600 mb-2">File Baru:</p>
                        ...
                        <div class="flex gap-2 sm:gap-3 pt-3 sm:pt-4 flex-shrink-0">
                            ...
                            <button type="submit" :disabled="isSubmitting"
                                ...
                                <i v-if="!isSubmitting" class="fa-solid fa-floppy-disk w-4 h-4"></i>
...
watch(
    () => props.modelValue,
    (isOpen) => {
        if (isOpen && props.articleData) {
            // Modal opened - load/reload data
            loadArticleData(props.articleData)
        } else if (!isOpen) {
...
// Watch for article data changes
watch(
    () => props.articleData,
    (newData) => {
        if (newData && props.modelValue) {
            loadArticleData(newData)
        }
    },
    { deep: true }
)

const loadArticleData = (article: ArticleData) => {
    formData.value = {
        judul: article.judul,
        tanggal: new Date(article.tanggal).toISOString().split('T')[0],
    ...
    imagePreview.value = article.gambar
    isNewImage.value = false
    existingFiles.value = article.files && Array.isArray(article.files) ? [...article.files] : []
    selectedFiles.value = []
    filesToDelete.value = []
...
const handleSubmit = async () => {
    ...
    if (!imagePreview.value) {
        submitError.value = 'Gambar artikel harus ada'
        return
    }
    ...
    try {
        ...
        // Compress and add new image if changed
        if (isNewImage.value && imagePreview.value) {
        ...
        if (filesToDelete.value.length > 0) {
            formDataObj.append('files_to_delete', JSON.stringify(filesToDelete.value))
        }

        // Compress and add new files
        ...
        if (!props.articleData?.id) {
            submitError.value = 'ID artikel tidak ditemukan'
            return
        }

        const result = await articleStore.editArticle(props.articleData.id, formDataObj)
        ...
        const message = error.message || 'Terjadi kesalahan saat memperbarui artikel'
