    ...
    >
      ...
      >
        ...
        >
          ...
          <div
            class="absolute top-0 right-0 w-40 h-40 bg-red-400/20 rounded-full blur-3xl -z-0"
          ></div>
          <div
            class="absolute bottom-0 left-0 w-32 h-32 bg-pink-400/20 rounded-full blur-3xl -z-0"
          ></div>
          ...
          <form @submit.prevent="handleSubmit" class="space-y-4 sm:space-y-6">
            ...
            <div>
              ...
              <input
                ...
                class="w-full px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg border-2 border-gray-200 text-sm sm:text-base focus:border-red-500 focus:ring-0 focus:outline-none transition-colors disabled:bg-gray-100 disabled:cursor-not-allowed"
                :disabled="isSubmitting"
              />
            </div>
            ...
            <div>
              ...
              <div
                v-if="
                  !imagePreview &&
                  saranaPrasaranaData?.foto &&
                  !shouldRemoveCurrentImage
                "
                class="mb-3 sm:mb-4 relative group"
              >
                <img
                  :src="saranaPrasaranaData.foto"
                  alt="Current Preview"
              ...
              >
                ...
                <div class="flex flex-col items-center gap-2 sm:gap-3">
                  ...
                  >
                    <svg
                      class="w-5 sm:w-6 h-5 sm:h-6 text-blue-600"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                  ...
                  <div>
                    ...
                    <p class="text-xs text-gray-600 mt-1">
                      Format: JPG, PNG, WebP (Maks. 5MB, akan dikompres ke 100KB)
                    </p>
                  </div>
                </div>
            ...
            >
              <div class="flex gap-2 sm:gap-3">
                <div class="flex-shrink-0">
                  <svg
                    class="h-4 sm:h-5 w-4 sm:w-5 text-red-600"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
                ...
                <div>
                  <p class="text-xs sm:text-sm font-semibold text-red-900">
                    Error
                  </p>
                  <p class="text-xs sm:text-sm text-red-800 mt-1">
                    {{ submitError }}
                  </p>
                </div>
              </div>
            ...
            <div class="flex gap-2 sm:gap-3 pt-3 sm:pt-4 flex-shrink-0">
              ...
              <button
                type="submit"
                :disabled="isSubmitting || !hasValidImage || !hasChanges"
              ...
              >
                <svg
                  v-if="isSubmitting"
                  class="w-3 sm:w-4 h-3 sm:h-4 animate-spin"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
...
import { updateSaranaPrasarana } from '~/services/sarana-prasarana'
import type { SaranaPrasaranaData } from '~/types/SaranaPrasaranaType'
import { compressImage, getFileSizeInMB, getFileSizeInKB } from '~/utils/imageCompressor'

interface Props {
  modelValue: boolean
  saranaPrasaranaData: SaranaPrasaranaData | null
}
...
const hasValidImage = computed(() => {
  ...
  if (props.saranaPrasaranaData?.foto && !shouldRemoveCurrentImage.value) {
    return true
  }
...
const hasChanges = computed(() => {
  if (!props.saranaPrasaranaData) return false

  // Check if name changed
  if (formData.value.name !== props.saranaPrasaranaData.name) {
    return true
  }
  ...
  if (
    ...
    (props.saranaPrasaranaData.status as 'active' | 'inactive')
  ) {
    return true
  }
...
// Initialize form with sarana prasarana data
watch(
  () => props.modelValue,
  (newVal) => {
    if (newVal && props.saranaPrasaranaData) {
      formData.value = {
        name: props.saranaPrasaranaData.name,
        status: props.saranaPrasaranaData.status as 'active' | 'inactive',
      }
...
const handleFileSelect = async (event: Event) => {
  ...
  if (file) {
    ...
    try {
      ...
      console.log(
        `Original: ${getFileSizeInKB(file.size)}KB → Compressed: ${getFileSizeInKB(compressedFile.size)}KB`
      )
    } catch (err) {
...
const handleSubmit = async () => {
  if (!props.saranaPrasaranaData) return
  ...
  try {
    const payload: any = {
      id: props.saranaPrasaranaData.id,
    ...
    await updateSaranaPrasarana(payload)

    // Success
    emit('success', 'Sarana prasarana berhasil diperbarui')
