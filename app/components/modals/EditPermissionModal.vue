      ...
      <div
        ...
        <div
          ...
          <div class="relative z-10 flex-1">
            <h2 class="text-lg sm:text-xl font-bold text-white">Edit Permission</h2>
          </div>
        ...
        <div class="p-4 sm:p-8 relative z-10 overflow-y-auto flex-1">
          ...
          <form @submit.prevent="handleSubmit" class="space-y-4 sm:space-y-6">
            <!-- Nama Permission Input -->
            <div>
              <label class="block text-xs sm:text-sm font-semibold text-gray-900 mb-2 sm:mb-3">
                Nama Hak Akses
                <span class="text-red-600 ml-1">*</span>
              </label>
              <input v-model="form.name" type="text" placeholder="Contoh: CREATE_USER" required
            ...
            <div>
              ...
              <textarea v-model="form.description" placeholder="Deskripsikan permission ini..." required
                :disabled="isSubmitting"
            ...
            <div>
              ...
              <input v-model="form.group_name" type="text" placeholder="Contoh: USER_MANAGEMENT" required
                :disabled="isSubmitting"
                class="w-full rounded-lg border-2 border-gray-300 bg-white px-4 py-2 sm:py-3 text-xs sm:text-sm font-medium transition-all duration-200 placeholder-gray-400 focus:border-red-600 focus:bg-white focus:outline-none focus:ring-4 focus:ring-red-100 disabled:opacity-50 disabled:cursor-not-allowed" />
            </div>
...
import type { SystemData, UpdatePermissionRequest } from '~/types/PermissionType'
...
// Form state
const form = ref<UpdatePermissionRequest>({
  id: 0,
  name: '',
  description: '',
  group_name: '',
...
const handleSubmit = async () => {
  ...
  if (!form.value.name || !form.value.description) {
    submitError.value = 'Nama hak akses dan deskripsi wajib diisi'
    return
  }

  if (!form.value.group_name) {
    submitError.value = 'Nama grup akses wajib diisi'
    return
  }
  ...
  try {
    ...
    console.log('Updating permission with request body:', requestBody)
    await updatePermission(requestBody)
    ...
    // Close modal
    emit('update:modelValue', false)
    emit('success', 'Permission berhasil diperbarui')

    resetForm()
  } catch (err: any) {
    const errorMessage = err.data?.message || err.data?.error || err.message || 'Gagal mengupdate permission'
...
onMounted(() => {
  ...
  // Populate form if permission data is provided
  if (props.permissionData) {
    form.value = {
      ...
      group_name: props.permissionData.group_name,
      system_id: props.permissionData.system_id,
