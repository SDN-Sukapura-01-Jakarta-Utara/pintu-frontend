            ...
            <div
                ...
                <div
                    ...
                    <div class="relative z-10 flex-1">
                        <h2 class="text-lg sm:text-xl font-bold text-white">Detail Pemetaan Rombel</h2>
                    </div>
                ...
                <div v-if="isLoading" class="p-4 sm:p-6 lg:p-8 flex items-center justify-center flex-1">
                    <div class="flex flex-col items-center gap-3 sm:gap-4">
                        ...
                        <p class="text-xs sm:text-sm md:text-base text-gray-600 font-medium">Memuat detail pemetaan...
                        </p>
                    </div>
                </div>

                <div v-else-if="pemetaan" class="p-3 sm:p-5 md:p-8 relative z-10 overflow-y-auto flex-1 space-y-6">
                    <!-- Pemetaan Info Card -->
                    <div
                        ...
                        <div class="flex items-start justify-between gap-2 sm:gap-3 mb-2 sm:mb-3">
                            <div class="min-w-0 flex-1">
                                <h3 class="text-sm sm:text-lg md:text-xl font-bold text-gray-900 truncate">{{
                                    pemetaan.peserta_didik?.nama }}</h3>
                            ...
                            <div :class="[
                                ...
                                pemetaan.status === 'active'
                                    ? 'bg-green-100 text-green-800'
                                    : 'bg-red-100 text-red-800'
                            ]">
                                {{ pemetaan.status === 'active' ? 'Aktif' : 'Nonaktif' }}
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 mt-3 sm:mt-4">
                            ...
                            <div class="flex items-center gap-2 sm:gap-2.5">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <p class="text-[11px] sm:text-xs font-semibold text-gray-900 truncate">{{
                                        pemetaan.tahun_pelajaran?.tahun_pelajaran || '-' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                                <div>
                                    ...
                                    <p class="text-xs sm:text-sm font-semibold text-gray-900">{{
                                        pemetaan.peserta_didik?.jenis_kelamin === 'L' ? 'Laki-laki' : 'Perempuan' }}</p>
                                </div>
                                <div>
                                    <p class="text-[11px] sm:text-xs text-gray-600 font-medium">Tempat Lahir</p>
                                    <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ pemetaan.peserta_didik?.tempat_lahir || '-' }}</p>
                                </div>
                                <div>
                                    <p class="text-[11px] sm:text-xs text-gray-600 font-medium">Tanggal Lahir</p>
                                    <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ formatDate(pemetaan.peserta_didik?.tanggal_lahir) }}</p>
                                </div>
                                <div>
                                    <p class="text-[11px] sm:text-xs text-gray-600 font-medium">Agama</p>
                                    <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ pemetaan.peserta_didik?.agama || '-' }}</p>
                                </div>
                                <div>
                                    <p class="text-[11px] sm:text-xs text-gray-600 font-medium">NIK</p>
                                    <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ pemetaan.peserta_didik?.nik || '-' }}</p>
                                </div>
                                <div>
                                    <p class="text-[11px] sm:text-xs text-gray-600 font-medium">Username</p>
                                    <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ pemetaan.peserta_didik?.username || '-' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="p-3 sm:p-4 md:p-6">
                            <div class="grid grid-cols-1 gap-3 sm:gap-4">
                                <div>
                                    ...
                                    <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ pemetaan.peserta_didik?.alamat || '-' }}</p>
                                </div>
                                <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                                    <div>
                                        <p class="text-[11px] sm:text-xs text-gray-600 font-medium">RT</p>
                                        <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ pemetaan.peserta_didik?.rt || '-' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-[11px] sm:text-xs text-gray-600 font-medium">RW</p>
                                        <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ pemetaan.peserta_didik?.rw || '-' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-[11px] sm:text-xs text-gray-600 font-medium">Kelurahan</p>
                                        <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ pemetaan.peserta_didik?.kelurahan || '-' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-[11px] sm:text-xs text-gray-600 font-medium">Kecamatan</p>
                                        <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ pemetaan.peserta_didik?.kecamatan || '-' }}</p>
                                    </div>
                                </div>
                                <div>
                                    <p class="text-[11px] sm:text-xs text-gray-600 font-medium">Kode Pos</p>
                                    <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ pemetaan.peserta_didik?.kode_pos || '-' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="p-3 sm:p-4 md:p-6">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                                <div>
                                    ...
                                    <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ pemetaan.peserta_didik?.nama_ayah || '-' }}</p>
                                </div>
                                <div>
                                    <p class="text-[11px] sm:text-xs text-gray-600 font-medium">Nama Ibu</p>
                                    <p class="text-xs sm:text-sm font-semibold text-gray-900">{{ pemetaan.peserta_didik?.nama_ibu || '-' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
