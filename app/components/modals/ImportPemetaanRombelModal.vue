            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl pointer-events-auto relative overflow-hidden max-h-[90vh] flex flex-col">
                ...
                <div class="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-base sm:text-lg font-bold text-gray-900">Import Pemetaan Rombel</h3>
                            <p class="text-xs sm:text-sm text-gray-600">Import data pemetaan rombel dari file Excel</p>
                        </div>
                    </div>
                ...
                <div class="flex-1 overflow-y-auto p-4 sm:p-6">
                    ...
                    <div class="mb-6">
                        ...
                        <button @click="downloadTemplate" :disabled="isDownloading || isLoading"
                            class="flex items-center gap-2 px-4 py-2.5 rounded-lg border-2 border-green-600 text-green-600 font-semibold text-sm hover:bg-green-50 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer">
                        ...
                        <p class="text-xs text-gray-600 mt-2">Download template Excel terlebih dahulu, lalu isi data pemetaan rombel sesuai format.</p>
                    </div>
                ...
                <div class="flex gap-3 p-4 sm:p-6 border-t border-gray-200 bg-gray-50">
                    ...
                    <button type="button" @click="handleImport" :disabled="!selectedFile || isLoading || isDownloading"
                        class="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-red-600 text-white font-semibold text-sm hover:bg-red-700 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer">
...
const downloadTemplate = async () => {
    ...
    try {
        const response = await fetch(`${config.public.apiBase}/api/v1/peserta-didik/download-template-pemetaan-rombel`, {
            method: 'POST',
            headers: {
        ...
        let filename = 'template-pemetaan-rombel.xlsx'
...
const handleImport = async () => {
    ...
    try {
        ...
        const response = await fetch(`${config.public.apiBase}/api/v1/peserta-didik/import-excel-pemetaan-rombel`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('auth_token') || ''}`,
            },
        ...
        if (!response.ok) {
            ...
            throw new Error(result.message || result.error || 'Gagal mengimport data')
        }

        isLoading.value = false

        // Check if there are errors in successful response
        if (result.data && result.data.errors && result.data.errors.length > 0) {
            // Show error modal
            importResult.value = result.data
            showErrorModal.value = true
            ...
            // All success
            toastStore.success('Sukses', `Berhasil mengimport ${result.data?.success_count || 0} data pemetaan rombel`)
        ...
        isLoading.value = false
        const errorMessage = error?.message || 'Gagal mengimport data pemetaan rombel'
        toastStore.error('Gagal', errorMessage)
    }
}
