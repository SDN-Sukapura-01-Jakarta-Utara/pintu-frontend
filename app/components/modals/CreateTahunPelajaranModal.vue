            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm pointer-events-auto overflow-hidden">
                ...
                <div class="p-6">
                    <p class="text-gray-700 text-sm sm:text-base mb-4">
                        Anda akan menyimpan tahun pelajaran <span class="font-semibold">{{ form.tahun_pelajaran
                            }}</span> dengan status <span class="font-semibold text-green-600">Aktif</span>.
                    </p>
                    <p class="text-gray-600 text-sm sm:text-base leading-relaxed">
                        Dengan menyimpan data ini, semua tahun pelajaran lainnya akan otomatis dinonaktifkan. Apakah
            ...
            <div
                ...
                <div
                    ...
                    <div class="relative z-10 flex-1">
                        <h2 class="text-lg sm:text-xl font-bold text-white">Tambah Tahun Pelajaran</h2>
                    </div>
...
const handleSubmit = async () => {
    ...
    // If status is active, show confirmation
    if (form.value.status === 'active') {
        showConfirmation.value = true
        return
    }

    // If status is inactive, submit directly
    await submitForm()
}
...
const submitForm = async () => {
    ...
    try {
        const { createTahunPelajaran } = await import('~/services/tahun-pelajaran')
        const requestBody = {
            tahun_pelajaran: form.value.tahun_pelajaran.trim(),
            status: form.value.status
        }

        console.log('Creating tahun pelajaran with request body:', requestBody)
        await createTahunPelajaran(requestBody)

        // Close modal
        emit('update:modelValue', false)
        emit('success', 'Tahun pelajaran berhasil ditambahkan')

        resetForm()
    } catch (err: any) {
        const errorMessage = err.data?.message || err.data?.error || err.message || 'Gagal menambahkan tahun pelajaran'
        submitError.value = errorMessage
        emit('error', errorMessage)
        console.error('Error creating tahun pelajaran:', err)
    } finally {
