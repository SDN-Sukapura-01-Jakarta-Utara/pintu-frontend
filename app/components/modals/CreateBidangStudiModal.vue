            ...
            <div
                ...
                <div
                    ...
                    <div class="relative z-10 flex-1">
                        <h2 class="text-lg sm:text-xl font-bold text-white">Tambah Bidang Studi</h2>
                    </div>
...
const submitForm = async () => {
    ...
    try {
        const { createBidangStudi } = await import('~/services/bidang-studi')
        const requestBody = {
            name: form.value.name.trim(),
            status: form.value.status
        }

        console.log('Creating bidang studi with request body:', requestBody)
        await createBidangStudi(requestBody)

        // Close modal
        emit('update:modelValue', false)
        emit('success', 'Bidang studi berhasil ditambahkan')

        resetForm()
    } catch (err: any) {
        const errorMessage = err.data?.message || err.data?.error || err.message || 'Gagal menambahkan bidang studi'
        submitError.value = errorMessage
        emit('error', errorMessage)
        console.error('Error creating bidang studi:', err)
    } finally {
