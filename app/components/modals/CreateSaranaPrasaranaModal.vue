        ...
        <div class="p-4 sm:p-8 relative z-10 overflow-y-auto flex-1">
          ...
          <form @submit.prevent="handleSubmit" class="space-y-4 sm:space-y-6">
            ...
            <div class="flex gap-2 sm:gap-3 pt-3 sm:pt-4 flex-shrink-0">
              ...
              <button
                type="submit"
                :disabled="!selectedFile || !formData.name.trim() || isSubmitting"
...
import { ref } from 'vue'
import { createSaranaPrasarana } from '~/services/sarana-prasarana'
import type { SaranaPrasaranaCreatePayload } from '~/types/SaranaPrasaranaType'
...
const handleSubmit = async () => {
  if (!selectedFile.value || !formData.value.name.trim()) {
    fileError.value = 'Nama dan foto harus diisi'
    return
  }
  ...
  try {
    const payload: SaranaPrasaranaCreatePayload = {
      file: selectedFile.value,
      name: formData.value.name.trim(),
      status: formData.value.status,
    }

    await createSaranaPrasarana(payload)

    // Success
    emit('success', 'Sarana prasarana berhasil ditambahkan')
    ...
    submitError.value =
      err.data?.message || err.message || 'Gagal menambahkan sarana prasarana'
