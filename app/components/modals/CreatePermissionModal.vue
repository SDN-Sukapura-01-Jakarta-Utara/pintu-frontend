      ...
      <div
        ...
        <div
          ...
          <div class="relative z-10 flex-1">
            <h2 class="text-lg sm:text-xl font-bold text-white">Tambah Permission</h2>
          </div>
...
import type { SystemData, CreatePermissionRequest } from '~/types/PermissionType'
...
const handleSubmit = async () => {
  ...
  try {
    const requestBody: CreatePermissionRequest = {
    ...
    console.log('Creating permission with request body:', requestBody)
    await createPermission(requestBody)
    ...
    // Close modal
    emit('update:modelValue', false)
    emit('success', 'Permission berhasil ditambahkan')

    resetForm()
  } catch (err: any) {
    const errorMessage = err.data?.message || err.data?.error || err.message || 'Gagal menambahkan permission'
