            ...
            <div
                ...
                <div
                    ...
                    <div class="relative z-10 flex-1">
                        <h2 class="text-lg sm:text-xl font-bold text-white">Tambah Peserta Didik</h2>
                    </div>
                    ...
                    <form @submit.prevent="handleSubmit" class="space-y-4 sm:space-y-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                            ...
                            <div>
                                ...
                                <input v-model="form.nis" type="text" placeholder="Nomor Induk Siswa" required
                                    :disabled="isSubmitting" @input="updateAutoPassword"
                                    class="w-full rounded-lg border-2 border-gray-300 bg-white px-4 py-2 sm:py-3 text-xs sm:text-sm font-medium transition-all duration-200 placeholder-gray-400 focus:border-red-600 focus:bg-white focus:outline-none focus:ring-4 focus:ring-red-100 disabled:opacity-50 disabled:cursor-not-allowed" />
                            </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                            ...
                            <div>
                                ...
                                <select v-model.number="form.rombel_id" required
                                    :disabled="isSubmitting || rombelLoading"
                                    class="w-full rounded-lg border-2 border-gray-300 bg-white px-4 py-2 sm:py-3 text-xs sm:text-sm font-medium transition-all duration-200 placeholder-gray-400 focus:border-red-600 focus:outline-none focus:ring-4 focus:ring-red-100 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer">
                            ...
                            <div>
                                ...
                                <select v-model.number="form.tahun_pelajaran_id" required
                                    :disabled="isSubmitting || tahunLoading"
                                    class="w-full rounded-lg border-2 border-gray-300 bg-white px-4 py-2 sm:py-3 text-xs sm:text-sm font-medium transition-all duration-200 placeholder-gray-400 focus:border-red-600 focus:outline-none focus:ring-4 focus:ring-red-100 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer">
...
const updateAutoPassword = () => {
    // For auto generate, username and password are set to NIS
}

/**
 * Extract error message from API response
 * Handles both single error message and field validation errors
 */
...
const handleSubmit = async () => {
    // Validate password type selection (should always have a value, default is 'auto')
    if (!form.value.passwordType || (form.value.passwordType !== 'auto' && form.value.passwordType !== 'manual')) {
        toastStore.error('Validasi', 'Pilih cara setting username dan password')
        return
    }
    ...
    try {
        const requestData = {
            ...
            username: form.value.passwordType === 'auto' ? form.value.nis : form.value.username,
            password: form.value.passwordType === 'auto' ? form.value.nis : form.value.password,
            role_ids: selectedRoles,
        }

        const result = await pesertaDidikStore.addPesertaDidik(requestData)

        if (result.success) {
            toastStore.success('Sukses', 'Peserta didik berhasil ditambahkan')
            closeModal()
            emit('success')
        } else {
...
const loadTahunPelajaran = async () => {
    ...
    try {
        ...
        if (result.success) {
            ...
            // Auto-select the active tahun pelajaran
            if (activeTahunPelajaran.value.length > 0) {
                form.value.tahun_pelajaran_id = activeTahunPelajaran.value[0].id
            }
        }
    } catch (err) {
...
/**
 * Watch untuk clear error state ketika modal ditutup
 */
watch(() => props.modelValue, (newVal) => {
    if (!newVal) {
        // Modal ditutup, clear error dari store
        pesertaDidikStore.clearError()
    }
})
</script>
