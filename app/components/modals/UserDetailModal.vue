            ...
            <div
                ...
                <div class="p-3 sm:p-5 md:p-8 relative z-10 overflow-y-auto flex-1">
                    ...
                    <div v-else-if="userDetail" class="space-y-6">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 mt-3 sm:mt-4">
                                <div class="flex items-center gap-2 sm:gap-2.5">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-[11px] sm:text-xs font-semibold text-gray-900 truncate">{{
                                            formatDate(userDetail.created_at) }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="flex items-center gap-2 sm:gap-2.5">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-[11px] sm:text-xs font-semibold text-gray-900 truncate">{{
                                            formatDate(userDetail.updated_at) }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div v-if="userDetail.roles && userDetail.roles.length > 0"
                            ...
                            <div class="divide-y divide-gray-200">
                                <div v-for="(groupData, systemName) in groupRolesBySystem" :key="systemName"
                                    class="p-2.5 sm:p-3 md:p-4 hover:bg-gray-50 transition-colors">
                                    <!-- System Header -->
                                    <div class="flex items-center gap-2 sm:gap-2.5 mb-2 sm:mb-3">
                                        <div
                                            class="flex-shrink-0 w-6 sm:w-8 h-6 sm:h-8 rounded-lg bg-gradient-to-br from-red-500 to-pink-500 flex items-center justify-center">
                                            <svg class="w-3 sm:w-4 h-3 sm:h-4 text-white" fill="none"
                                                stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        <div class="min-w-0">
                                            <h4 class="text-xs sm:text-sm font-bold text-gray-900 truncate">{{
                                                systemName }}</h4>
                                            <p class="text-[11px] sm:text-xs text-gray-500">{{ groupData.system?.description || 'Sistem Informasi' }}</p>
                                        </div>
                                    </div>

                                    <!-- Roles List -->
                                    <div class="ml-9 sm:ml-11 space-y-1.5 sm:space-y-2">
                                        <div v-for="role in groupData.roles" :key="role.id"
                                            class="flex items-start gap-1.5 sm:gap-2 p-1.5 sm:p-2 bg-gray-50 rounded-lg">
                                            <div
                                                class="flex-shrink-0 w-4 sm:w-5 h-4 sm:h-5 rounded-full bg-green-200 flex items-center justify-center mt-0.5 flex-shrink-0">
                                                <svg class="w-2.5 sm:w-3 h-2.5 sm:h-3 text-green-600"
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <p class="text-xs sm:text-xs font-semibold text-gray-900 truncate">
                                                    {{ role.name }}</p>
                                                <p class="text-[11px] sm:text-xs text-gray-600 mt-0.5 line-clamp-2">{{
                                                    role.description }}</p>
                                                <div class="flex items-center gap-1.5 mt-0.5 sm:mt-1 flex-wrap">
                                                    <span v-if="role.status" :class="[
                                                        'inline-flex items-center px-1 sm:px-1.5 py-0.5 rounded-full text-[10px] sm:text-[10px] font-semibold',
                                                        role.status === 'active'
                                                            ? 'bg-green-100 text-green-800'
                        ...
                        <!-- No Roles State -->
                        <div v-else
                            class="bg-yellow-50 rounded-lg sm:rounded-xl border-2 border-yellow-200 p-2.5 sm:p-3 md:p-4">
                            <div class="flex items-start gap-2 sm:gap-3">
                                <div class="flex-shrink-0">
                                    <svg class="h-4 sm:h-5 w-4 sm:w-5 text-yellow-600" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <div>
                                    <h3 class="text-[11px] sm:text-sm font-semibold text-yellow-900">Tidak ada role</h3>
                                    <p class="mt-0.5 text-[10px] sm:text-xs text-yellow-800">User ini belum memiliki
                                        role yang ditugaskan</p>
                                </div>
                            </div>
                        </div>
                    </div>
...
// Group roles by system with system info
const groupRolesBySystem = computed(() => {
    if (!userDetail.value?.roles) return {}

    const grouped: Record<string, { roles: any[]; system: any }> = {}
    userDetail.value.roles.forEach((role: any) => {
        ...
        grouped[systemName].roles.push(role)
    })
