            ...
            <div
                ...
                <div class="p-4 sm:p-6">
                    <div class="flex flex-col items-center text-center">
                        <!-- Warning Icon -->
                        <div
                            class="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-red-100 flex items-center justify-center mb-3 sm:mb-4">
                        ...
                        <!-- Title -->
                        <h3 class="text-base sm:text-lg font-bold text-gray-900 mb-2">Hapus Pemetaan Rombel</h3>

                        <!-- Message -->
                        <p class="text-xs sm:text-sm text-gray-600 mb-4 sm:mb-6">
                            Apakah Anda yakin ingin menghapus pemetaan rombel untuk siswa <span class="font-semibold">'{{ siswaName }}'</span>?
                            <br>
                    ...
                    <div class="flex gap-3">
                        ...
                        <button type="button" @click="handleConfirm" :disabled="isLoading"
                            class="flex-1 flex items-center justify-center gap-2 px-4 py-2 sm:py-2.5 rounded-lg bg-red-600 text-white font-semibold text-xs sm:text-sm hover:bg-red-700 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer">
                            <span v-if="isLoading"
...
const handleConfirm = async () => {
    if (!props.pemetaanId) return

    isLoading.value = true

    try {
        await $fetch(`${config.public.apiBase}/api/v1/peserta-didik/delete-pemetaan-rombel-by-id`, {
            method: 'POST',
            headers: {
        ...
        toastStore.success('Sukses', 'Pemetaan rombel berhasil dihapus')
        
        // Close modal first
        emit('update:modelValue', false)
        
        // Then emit success after modal closed
        await nextTick()
        emit('success')
    } catch (error: any) {
        console.error('Error deleting pemetaan rombel:', error)
        isLoading.value = false
        ...
        const errorMessage = error?.data?.message || error?.message || 'Gagal menghapus pemetaan rombel'
